...
@Component({
  ...
  styles: `
  .ease-pop {
    --tw-ease: cubic-bezier(.16,2,.68,.96);
    transition-timing-function: cubic-bezier(.16,2,.68,.96);
  }
  `
})
export class LandingComponent implements AfterViewInit {
  ...
  initializeDraggableImage() {
    ...
    interact(this.draggableImage.nativeElement)
      .draggable({
...
class Button { 
  ...
  private getExitCoordinates(e: MouseEvent) {
    const { left, top, right, bottom, width, height } = this.DOM.button.getBoundingClientRect();
