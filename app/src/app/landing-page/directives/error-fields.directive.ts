...
export class ErrorFieldsDirective implements OnInit {
  ...
  constructor(private elRef: ElementRef, private control: NgControl, private renderer: Renderer2) {
   }
  ngOnInit(): void {
        ...
        this.renderer.setAttribute(this.iconElement, 'src', 'assets/images/danger-icon.svg');
  ...
  private updateLabelColor(state: 'valid' | 'invalid' | 'neutral'): void {
    ...
    if (this.elRef.nativeElement.parentElement) {
      const spans = this.elRef.nativeElement.parentElement.querySelectorAll('span');
...
private getErrorMessage() : string {
  ...
  if (errorKey === 'pattern') {
    // Intentar determinar tipo de campo para mensaje más específico
    const inputType = this.elRef.nativeElement.type;
    if (inputType === 'email' || this.elRef.nativeElement.id.includes('email')) {
